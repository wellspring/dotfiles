...
; Description: Lightens and mutes colours while applying a teal filter to 
...
(define (script-fu-dawn-urbane    	image
									layer
	)
...
(let*	(
	(red-div (car (gimp-layer-new-from-drawable layer image)))
	)

	(gimp-layer-set-name red-div "Red Divide")
	(gimp-context-set-foreground '(255 0 0))
	(gimp-drawable-fill red-div FOREGROUND-FILL) 
    (gimp-image-add-layer image red-div 0)
    (gimp-layer-set-mode red-div 15)
    (gimp-layer-set-opacity red-div 10))	
...
(script-fu-register 
	"script-fu-dawn-urbane"
	"Dawn Urbane"
	"Lightens and mutes colours while applying a teal filter to dark photos and images."
	"Claire Jones"
	"© 2008 Claire Jones"
	"September 3, 2008"
	"RGB*"
...
(script-fu-menu-register "script-fu-dawn-urbane" "<Image>/Filters/Colors/Dawn")		
