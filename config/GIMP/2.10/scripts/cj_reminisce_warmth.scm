...
(define (script-fu-reminisce-warmth    	image
										layer
	)
...
(let*	(
    ...
    (gimp-layer-set-opacity beige-soft 70))

(let*	(
	(brown-hard (car (gimp-layer-new-from-drawable layer image)))
	)

	(gimp-layer-set-name brown-hard "Brown Hard Light")
	(gimp-context-set-foreground '(119 69 43))
	(gimp-drawable-fill brown-hard FOREGROUND-FILL) 
    (gimp-image-add-layer image brown-hard 0)
    (gimp-layer-set-mode brown-hard 18)
    (gimp-layer-set-opacity brown-hard 20))
...
(script-fu-register 
    ...
    (script-fu-menu-register "script-fu-reminisce-warmth" "<Image>/Filters/Colors/Reminisce")		
