...
; Description: Lightens, mutes colours and applies a bluish-green filter to 
...
(define (script-fu-dawn-prelude    	image
									layer
	)
...
(let*	(
	(green-soft (car (gimp-layer-new-from-drawable layer image)))
	)

	(gimp-layer-set-name green-soft "Green Soft Light")
	(gimp-context-set-foreground '(0 255 0))
	(gimp-drawable-fill green-soft FOREGROUND-FILL) 
    (gimp-image-add-layer image green-soft 0)
    (gimp-layer-set-mode green-soft 19)
    (gimp-layer-set-opacity green-soft 20))		
...
(script-fu-register 
	"script-fu-dawn-prelude"
	"Dawn Prelude"
	"Lightens, mutes colours and applies a bluish-green filter to dark photos and images."
	"Claire Jones"
	"© 2008 Claire Jones"
	"September 26, 2008"
	"RGB*"
...
(script-fu-menu-register "script-fu-dawn-prelude" "<Image>/Filters/Colors/Dawn")		
