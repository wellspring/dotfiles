...
group.commands.add(['tinyurl'], 'echo and copy TinyURL',
    function(args) {
        var tinyurl = tiny.getTiny(args.length===0 ? buffer.URL : args.string);
        dactyl.clipboardWrite(tinyurl, true);
        // dactyl.echo("TinyURL: " + tinyurl);
    }, {argCount:'?'});

group.commands.add(['expandurl'], 'expand TinyURL',
    function(args) {
        var expandurl = tiny.getExpand(args.string);
        dactyl.clipboardWrite(expandurl, true);
        // dactyl.echo("ExpandURL: " + expandurl);
    }, {argCount:'1'});
